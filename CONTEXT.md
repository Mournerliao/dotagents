...
| Inclusion mode | How a catalog entry joins the sync set: `maintained` (canonical source hosted here) or `delegated` (an install recipe that invokes an upstream installer). Chosen per entry; prefer `delegated` when a reliable upstream installer exists. |
...
| Managed installation | Legacy concept: a local lock record of installer-owned files. **Not part of the intended personal-library model** — this library does not persist per-machine install inventories; operators (human or agent) discover current installs from the environment when needed. |
